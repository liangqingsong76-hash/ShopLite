...
class CheckoutFlowTests(TestCase):
        ...
        paid_order, changed = handle_payment_notification(
            {
                ...
                "sign": self._payment_sign(order.order_no, "TRADE-2", str(order.pay_amount)),
            }
        )
    ...
    @staticmethod
    def _payment_sign(order_no, trade_no, amount):
        """生成与模拟支付网关一致的测试签名。"""

        message = f"{order_no}|{trade_no}|{amount}".encode("utf-8")
        return hmac.new(b"test-secret", message, "sha256").hexdigest()
...
class PhoneVerificationTests(TestCase):
    ...
    def test_send_login_code_rejects_unregistered_phone(self):
        ...
        # 防账号枚举：外部响应与已注册号码保持一致，但不会实际签发登录验证码。
        self.assertEqual(response.status_code, 200)
...
class WechatLoginTests(TestCase):
    def test_mock_wechat_login_is_disabled(self):
        response = self.client.get("/api/auth/wechat/login/")

        self.assertEqual(response.status_code, 503)
        self.assertFalse(SocialAccount.objects.filter(provider="weixin").exists())
        self.assertIsNone(self.client.session.get("_auth_user_id"))

    def test_repeated_mock_wechat_login_does_not_create_account(self):
