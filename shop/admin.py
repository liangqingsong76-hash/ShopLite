...
class CategoryAdmin(ProtectedDeleteAdminMixin, admin.ModelAdmin):
...
class ProductAdmin(ProtectedDeleteAdminMixin, admin.ModelAdmin):
    list_display = (
        "image_thumb",
        "name",
        "category_path",
...
class OrderAdmin(ProtectedDeleteAdminMixin, admin.ModelAdmin):
...
class OrderItemAdmin(ProtectedDeleteAdminMixin, admin.ModelAdmin):
...
class UserProfileAdmin(ProtectedDeleteAdminMixin, admin.ModelAdmin):
...
class CouponAdmin(ProtectedDeleteAdminMixin, admin.ModelAdmin):
...
class RefundRequestAdmin(ProtectedDeleteAdminMixin, admin.ModelAdmin):
    ...
    def approve_refunds(self, request, queryset):
        ...
        for refund in queryset:
            try:
                _, did_change = approve_refund_request(refund)
                changed += int(did_change)
            except ValidationError:
                failed += 1
        self.message_user(
            request,
            f"已同意 {changed} 条，跳过/失败 {failed} 条",
    ...
    def reject_refunds(self, request, queryset):
        ...
        for refund in queryset:
            try:
                _, did_change = reject_refund_request(refund)
                changed += int(did_change)
            except ValidationError:
                failed += 1
        self.message_user(
            request,
            f"已拒绝 {changed} 条，跳过/失败 {failed} 条",
            messages.SUCCESS if not failed else messages.WARNING,
        )
...
class ProductSourceAdmin(ProtectedDeleteAdminMixin, admin.ModelAdmin):
    """只读展示商品外部来源与导入追溯信息。"""

    list_display = ("source_type", "external_id", "product", "last_imported_at")
    list_filter = ("source_type", "last_imported_at")
    search_fields = ("external_id", "product__name", "source_url")
    list_select_related = ("product",)
    readonly_fields = (
        "product",
        "source_type",
