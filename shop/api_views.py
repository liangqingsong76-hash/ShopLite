...
def cart_add(request):
    ...
    try:
        product_id = _positive_id(data.get("product_id"), "商品 ID", required=True)
        color = _bounded_string(data.get("color", ""), "颜色", max_length=50)
        specs = _bounded_string(data.get("specs", ""), "规格", max_length=200)
...
def cart_update(request):
    ...
    if data is None:
        return JsonResponse({"error": "无效请求"}, status=400)
    try:
        item_id = _positive_id(data.get("item_id"), "购物车项 ID", required=True)
...
def cart_delete(request):
    ...
    if not deleted:
        return JsonResponse({"error": "购物车项不存在"}, status=404)
    count = CartItem.objects.filter(user=request.user).aggregate(total=Count("id"))["total"] or 0
...
def order_create(request):
    ...
    try:
        address_id = _positive_id(data.get("address_id"), "地址 ID")
        user_coupon_id = _positive_id(data.get("user_coupon_id"), "优惠券 ID")
...
def _positive_id(value, label, *, required=False):
    """把外部 ID 收紧为数据库可接受的正整数。"""
...
def _bounded_string(value, label, *, max_length):
    """校验来自 JSON 的短文本，拒绝嵌套对象和超长内容。"""
