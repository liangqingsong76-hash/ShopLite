...
def paginated_product_context(request, products, *, per_page=24):
    """把商品查询集转换为保留当前筛选参数的分页上下文。"""

    page = Paginator(products, per_page).get_page(request.GET.get("page"))
    pagination_params = request.GET.copy()
    pagination_params.pop("page", None)
    return {"products": page, "pagination_query": pagination_params.urlencode()}
...
def new_products(request):
    return render(
        request,
        "new_products.html",
        page_context(request, **paginated_product_context(request, product_queryset(new=True))),
    )
...
def hot_products(request):
    return render(
        request,
        "hot_products.html",
        page_context(request, **paginated_product_context(request, product_queryset(hot=True))),
    )
...
def brand_page(request):
    return render(
        ...
        page_context(
            ...
            **paginated_product_context(request, product_queryset(recommended=True)),
        ),
    )
...
def category(request):
    ...
    pagination = paginated_product_context(request, products)
