...
class CommerceSecurityTests(TestCase):
    ...
    @staticmethod
    def _payment_payload(order, trade_no):
        """构造带有效 HMAC 的模拟支付回调。"""

        amount = str(order.pay_amount)
        message = f"{order.order_no}|{trade_no}|{amount}".encode("utf-8")
        return {
            "out_trade_no": order.order_no,
            "trade_no": trade_no,
            "total_amount": amount,
            "sign": hmac.new(b"test-secret", message, "sha256").hexdigest(),
        }
    ...
    def test_payment_requires_amount_and_rejects_reused_trade_number(self):
        ...
        handle_payment_notification(self._payment_payload(first, "TX-1"))
        CartItem.objects.create(user=self.user, product=self.product, quantity=1)
        second = create_order_from_cart(self.user, address_id=self.address.id)
        with self.assertRaises(ValidationError):
            handle_payment_notification(self._payment_payload(second, "TX-1"))

    def test_refund_completion_stays_disabled_without_real_gateway(self):
...
class PageAndProfileTests(TestCase):
    ...
    def test_empty_category_renders_empty_state(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertEqual(list(response.context["products"].object_list), [])
...
class AdminPageTests(TestCase):
    ...
    def test_core_admin_change_lists_render(self):
        urls = (
            ...
            reverse("admin:commerce_coupon_changelist"),
            reverse("admin:payments_paymenttransaction_changelist"),
            reverse("admin:commerce_refundrequest_changelist"),
