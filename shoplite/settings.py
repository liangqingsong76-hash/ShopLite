...
# allauth 保留用户名和已验证邮箱登录；新账号注册由 accounts 的手机号短信流程负责。
ACCOUNT_ADAPTER = "accounts.adapters.PhoneOnlyAccountAdapter"
...
WECHAT_APP_ID = env_value("WECHAT_APP_ID", default="").strip()
WECHAT_APP_SECRET = env_value("WECHAT_APP_SECRET", default="").strip()


def _build_wechat_socialaccount_providers(mode, app_id, app_secret):
    """把环境凭据转换为 allauth 的 settings-backed SocialApp 配置。"""

    normalized_mode = str(mode or "").strip().lower()
    if normalized_mode != "allauth":
        return {}
    if not app_id or not app_secret:
        raise RuntimeError(
            "WECHAT_LOGIN_MODE=allauth 时必须配置 WECHAT_APP_ID 和 WECHAT_APP_SECRET"
        )
    return {
        "weixin": {
            "APP": {
...
SOCIALACCOUNT_PROVIDERS = _build_wechat_socialaccount_providers(
    WECHAT_LOGIN_MODE,
    WECHAT_APP_ID,
    WECHAT_APP_SECRET,
)

# 智能找商品 provider 留空时使用本地文字规则，并安全停用图片识别。
DISCOVERY_TEXT_INTENT_PROVIDER = env_value("DISCOVERY_TEXT_INTENT_PROVIDER", default="").strip()
DISCOVERY_IMAGE_INTENT_PROVIDER = env_value("DISCOVERY_IMAGE_INTENT_PROVIDER", default="").strip()
DISCOVERY_TEXT_RATE_LIMIT = int(env_value("DISCOVERY_TEXT_RATE_LIMIT", default="30"))
DISCOVERY_VOICE_RATE_LIMIT = int(env_value("DISCOVERY_VOICE_RATE_LIMIT", default="30"))
DISCOVERY_IMAGE_RATE_LIMIT = int(env_value("DISCOVERY_IMAGE_RATE_LIMIT", default="10"))
DISCOVERY_RATE_LIMIT_WINDOW_SECONDS = int(
    env_value("DISCOVERY_RATE_LIMIT_WINDOW_SECONDS", default="300")
)

# 头像上限为 5 MB；图片检索端点仍在自身边界内限制为 2 MB。
FILE_UPLOAD_MAX_MEMORY_SIZE = 5 * 1024 * 1024 + 64 * 1024
